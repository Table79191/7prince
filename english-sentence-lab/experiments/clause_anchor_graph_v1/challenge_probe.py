...
# Unambiguous head-role checks under SentenceLab's head-only school-style SVOC convention.
FOCUS = {
    2:"S", 3:"O", 5:"S", 7:"V", 9:"V",
...
def main():
    ...
    ap.add_argument("--model", default=str(Path(__file__).resolve().parent/"artifacts/mini_clause_anchor_graph_v1.pt"))
    ap.add_argument("--out", default=str(Path(__file__).resolve().parent/"artifacts/challenge_probe.json"))
    args = ap.parse_args()

    ck = torch.load(args.model, map_location="cpu", weights_only=False)
    model = ClauseAnchorGraph()
    ...
    for idx, gold in FOCUS.items():
        ...
        checks.append({
            ...
            "pred": pred,
            "ok": ok,
            "owner_index": result.owners[idx],
            "owner_token": TOKENS[result.owners[idx]],
        })
    ...
    payload = {
        ...
        "clause_heads": result.clause_heads,
        "clauses": clauses,
        "focus_correct": correct,
        "focus_total": len(FOCUS),
        "focus_accuracy": correct / len(FOCUS),
        "checks": checks,
        "training_metrics": ck.get("metrics", {}),
    }
    Path(args.out).parent.mkdir(parents=True, exist_ok=True)
    Path(args.out).write_text(json.dumps(payload, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
    print("CLAUSE_ANCHOR_CHALLENGE="+json.dumps(payload, ensure_ascii=False))
