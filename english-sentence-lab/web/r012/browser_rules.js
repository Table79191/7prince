(function(root){
  ...
  function expandContractions(text){
    ...
    s=s.replace(/\b([A-Za-z]+)'re\b/gi,'$1 are');
    s=s.replace(/\b([A-Za-z]+)'ve\b/gi,'$1 have');
    s=s.replace(/\b([A-Za-z]+)'ll\b/gi,'$1 will');
    s=s.replace(/\b([A-Za-z]+)'m\b/gi,'$1 am');
    return s;
  }

  function splitTokens(text){
    const s=expandContractions(text);
    return s.match(/[A-Za-z]+|'(?:s|d)|\d+(?:[.,]\d+)?|[^\sA-Za-z0-9]/g)||[];
  }
  ...
  function inferPos(tokens){
    ...
    for(let i=0;i<tokens.length;i++){
      ...
      else if(ADJ.has(w)||/(ous|ful|less|ive|able|ible|al|ic|ary|ory)$/.test(w)) p='ADJ';
  ...
  function postprocessRoles(tokens,pos,inputRoles){
    ...
    const lo=tokens.map(x=>x.toLowerCase());

    const setRole=(i,r)=>{ if(i>=0&&i<roles.length) roles[i]=r; };

    for(let i=0;i<lo.length;i++){
      if(lo[i]!=='not') continue;
      setRole(i,'M');
      if(i>0 && (pos[i-1]==='AUX'||AUX.has(lo[i-1]))) setRole(i-1,'V');
    ...
    // Direct copular WH inversion: What the hell was that?
    let cop=-1;
    for(let i=0;i<lo.length;i++){
      if(COPULAS.has(lo[i]) && (pos[i]==='AUX'||pos[i]==='VERB')){cop=i;break;}
    }
    if(cop>0 && WH.has(lo[0])){
      setRole(0,'C');
      if(lo[0]==='what' && lo[1]==='the' && lo[2]==='hell' && cop>=3){
        setRole(1,'M'); setRole(2,'M');
      }else{
        for(let i=1;i<cop;i++) if(pos[i]!=='PUNCT') setRole(i,'C');
      }
      setRole(cop,'V');
    ...
    // Final school-style noun phrase rule: only the nominal head keeps S/O/C.
    const isNum=w=>/^\d+(?:\.\d+)?$/.test(w.replace(/,/g,''));
    const isModifier=i=>{
      const w=lo[i], p=pos[i];
      return POSSESSIVE_DET.has(w)||DETS.has(w)||isNum(w)||w==='%'||
             ['DET','ADJ','NUM','SYM'].includes(p)||w==="'s";
    };
    ...
    // A post-copular demonstrative alone is a pronoun subject.
    if(cop>=0){
      for(let i=cop+1;i<lo.length;i++){
        if(pos[i]==='PUNCT') break;
        if(DEMONSTRATIVES.has(lo[i]) && (i+1===lo.length||pos[i+1]==='PUNCT')){
          pos[i]='PRON'; setRole(i,'S');
        }
        break;
      }
    }
