(function(root){
  ...
  function expandContractions(text){
    ...
    return s.replace(/\b([A-Za-z]+)n't\b/gi,'$1 not').replace(/\b([A-Za-z]+)'re\b/gi,'$1 are').replace(/\b([A-Za-z]+)'ve\b/gi,'$1 have').replace(/\b([A-Za-z]+)'ll\b/gi,'$1 will').replace(/\b([A-Za-z]+)'m\b/gi,'$1 am');
  }
  ...
  function inferPos(tokens){
    ...
    for(let i=0;i<tokens.length;i++){
      ...
      if(raw==='%')p='SYM';else if(/^[^\w']+$/.test(raw))p='PUNCT';else if(/^\d+(?:[.,]\d+)?$/.test(raw))p='NUM';
      else if(w==='not')p='PART';else if(w==="'d")p='AUX';
      else if(w==="'s")p=(ADJ.has(next)||looksVerb(next)||ADV.has(next)||next==='not')?'AUX':'PART';
      else if(AUX.has(w))p='AUX';else if(POSSESSIVE_DET.has(w))p='DET';else if(PRONOUNS.has(w))p='PRON';else if(DETS.has(w))p='DET';
      else if(CCONJ.has(w))p='CCONJ';else if(SCONJ.has(w))p='SCONJ';else if(w==='to')p='PART';else if(ADP.has(w))p='ADP';else if(INTJ.has(w))p='INTJ';
      else if(ADV.has(w)||/ly$/.test(w))p='ADV';else if(ADJ.has(w))p='ADJ';else if(VERBS.has(w)||/(ing|ed|ize|ise|ify)$/.test(w))p='VERB';
      else if(/(ous|ful|less|ive|able|ible|al|ic|ary|ory)$/.test(w))p='ADJ';else if(i>0&&/^[A-Z]/.test(raw))p='PROPN';else p='NOUN';
      out.push(p);
    }return out;
  }
  function weakRoles(pos){const out=[];let seen=false;for(const p of pos){if(p==='VERB'||p==='AUX'){out.push('V');seen=true;}else if(p==='PUNCT')out.push(null);else if(p==='NOUN'||p==='PROPN'||p==='PRON')out.push(seen?'O':'S');else out.push('M');}return out;}
  function postprocessRoles(tokens,pos,inputRoles){
    ...
    for(let i=0;i<lo.length;i++){if(lo[i]!=='not')continue;setRole(i,'M');if(i>0&&(pos[i-1]==='AUX'||AUX.has(lo[i-1])||lo[i-1]==="'s"||lo[i-1]==="'d"))setRole(i-1,'V');for(let j=i+1;j<lo.length;j++){if(pos[j]==='PUNCT')break;if(pos[j]==='ADV'||pos[j]==='PART')continue;if(pos[j]==='VERB'||['be','have','do'].includes(lo[j]))setRole(j,'V');break;}}
    for(let i=0;i<lo.length;i++){if((lo[i]==="'s"||lo[i]==="'d")&&pos[i]==='AUX')setRole(i,'V');if(lo[i]==="'s"&&pos[i]==='PART')setRole(i,'M');}
    let cop=-1;for(let i=0;i<lo.length;i++){if(COPULAS.has(lo[i])&&(pos[i]==='AUX'||pos[i]==='VERB')){cop=i;break;}}
    if(cop>0&&WH.has(lo[0])){setRole(0,'C');for(let i=1;i<cop;i++)if(pos[i]!=='PUNCT')setRole(i,'M');setRole(cop,'V');for(let i=cop+1;i<lo.length;i++){if(pos[i]==='PUNCT')break;setRole(i,'S');}}
    const isNum=w=>/^\d+(?:\.\d+)?$/.test(w.replace(/,/g,''));const isModifier=i=>{const w=lo[i],p=pos[i];return POSSESSIVE_DET.has(w)||DETS.has(w)||isNum(w)||w==='%'||['DET','ADJ','NUM','SYM'].includes(p)||(w==="'s"&&p==='PART');};
    for(let head=0;head<tokens.length;head++){if(!['NOUN','PROPN','PRON'].includes(pos[head])||!['S','O','C'].includes(roles[head]))continue;for(let j=head-1;j>=0;j--){if(/^[,.!?;:]$/.test(tokens[j])||!isModifier(j))break;setRole(j,'M');}}
    if(cop>=0){for(let i=cop+1;i<lo.length;i++){if(pos[i]==='PUNCT')break;if(DEMONSTRATIVES.has(lo[i])&&(i+1===lo.length||pos[i+1]==='PUNCT')){pos[i]='PRON';setRole(i,'S');}break;}}
    return roles;
  }
  const api={expandContractions,splitTokens,inferPos,weakRoles,postprocessRoles};if(typeof module!=='undefined'&&module.exports)module.exports=api;root.SentenceLabBrowserRules=api;
